...
def main():
    ...
    height = abs( int(str(args.resolution).split("x")[0]) )
    width = abs( int(str(args.resolution).split("x")[1]) )
    destinationFilename = args.output

    # Define the mode we are in, and target resolution
    print('Tile mode is {0}'.format(args.mode))
    print('Target resolution {0}x{1}'.format(height,width))
    ...
    for sourceFilename in filesToProcess:
        ...
        if canvas:
            ...
            if args.mode == 'rotate':
                ...
                canvas = scaleCanvas(canvas,height, width)
...
def parseArgs():
    ...
    parser.add_argument("-r",'--resolution', default="240x240", type=str, help='resolution hxw of target device')
...
def processAngle(canvas,fontCanvas, angleStart, angleStop, angleSteps):
        ...
        for angle in range(angleStart, angleStop+angleSteps, angleSteps):
            ...
            currentCanvas = currentCanvas.transpose(Image.FLIP_LEFT_RIGHT)
            currentCanvas = scaleCanvas(currentCanvas,height,width)
...
def scaleCanvas(canvas,newh, neww):
    if (canvas.size[0] == newh and canvas.size[1] == neww):
        return canvas
    else:
        return canvas.resize((newh,neww), Image.LANCZOS)
